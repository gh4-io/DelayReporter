...
try {
    ...
    # ---- Outlook on the web ------------------------------------------------
    Write-Output ''
    Write-Output '== Outlook on the web =='
    $webNotes = New-Object 'System.Collections.Generic.List[string]'
    $url = [DelayReporter.Core.Email.OutlookWeb]::ComposeUrl($draft, [DelayReporter.Core.Email.EmailClient]::OutlookWeb, $webNotes)
    Assert ($url.StartsWith('https://outlook.office.com/mail/deeplink/compose?to=ops%40example.com&subject=CVG%20departure')) `
        "the compose link carries the recipient and subject: $url"
    AssertEqual 1 $webNotes.Count 'and leaves out an address that is not one, with a note'
    $personal = [DelayReporter.Core.Email.OutlookWeb]::ComposeUrl($draft, [DelayReporter.Core.Email.EmailClient]::OutlookCom, $webNotes)
    Assert ($personal.StartsWith('https://outlook.live.com/')) 'a personal account uses Outlook.com'

    $clip = [DelayReporter.Core.Email.ClipboardHtml]::Format($draft.Html)
    $clipBytes = [Text.Encoding]::UTF8.GetBytes($clip)
    function ClipOffset($name) { [int]([regex]::Match($clip, $name + ':(\d{10})').Groups[1].Value) }
    AssertEqual $clipBytes.Length (ClipOffset 'EndHTML') 'the clipboard header measures the document in UTF-8 bytes'
    $fragment = [Text.Encoding]::UTF8.GetString($clipBytes, (ClipOffset 'StartFragment'),
        (ClipOffset 'EndFragment') - (ClipOffset 'StartFragment'))
    Assert ($fragment.StartsWith('<div style="font-family:') -and $fragment.EndsWith('</div>')) `
        'the pasted fragment keeps the email font'
    Assert ($fragment -match 'ZZ104' -and -not ($fragment -match '<body')) 'and is the table without the page around it'

    # ---- built-in presets --------------------------------------------------
    Write-Output ''
    Write-Output '== built-in presets =='
    AssertEqual 'Standard|All|MX only|Every coded delay|Over an hour' ([DelayReporter.Core.BuiltInPresets]::Names -join '|') `
        'Standard, All and MX only come first'
    $carriers = [DelayReporter.Core.BuiltInPresets]::StationOperators
    AssertEqual '3S,CJT,CKS,CSB,DHK,KII,SIA' ($carriers -join ',') 'the station has seven default carriers, 3S among them'
    $standard = [DelayReporter.Core.BuiltInPresets]::Build('Standard')
    AssertEqual ($carriers -join ',') ($standard.Operators -join ',') 'Standard ticks them, as a freshly opened file does'
    AssertEqual 15 $standard.MinimumDelayMinutes 'at 15 minutes'
    AssertEqual 'All' $standard.MxFilter.ToString() 'with every flight, MX or not'
    $all = [DelayReporter.Core.BuiltInPresets]::Build('All')
    AssertEqual 0 $all.Operators.Count 'All restricts no operator'
    AssertEqual 15 $all.MinimumDelayMinutes 'at the same 15 minutes'
    $mxOnly = [DelayReporter.Core.BuiltInPresets]::Build('MX only')
    Assert (($mxOnly.Operators -join ',') -eq ($carriers -join ',') -and $mxOnly.MinimumDelayMinutes -eq 15) 'MX only starts from Standard'
    AssertEqual 'MxOnly' $mxOnly.MxFilter.ToString() 'and reports MX delays only'
