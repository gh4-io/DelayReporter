...
(Superseded after 0.4.0 by "Continuous integration after all", below.)
...
## Standard is how a file opens

A file opens with the station's own carriers ticked under Operators, while the Standard preset used
to tick every operator, so applying Standard to a freshly opened file changed it. Standard now is
the opening state, and both go through the same code: the carriers live once, in
`BuiltInPresets.StationOperators`, and the window ticks them on opening by the same route a preset
takes. The old Standard, every operator, is kept as **All**, and **MX only** is Standard with the
MX filter set. A carrier list typed in two places had already drifted once (`S3` for `3S`); a
single list cannot.

The Operators list offers only what the file holds, so a carrier the file lacks cannot be ticked,
and showing it ticked but greyed would only make the count harder to read. Instead the list names
what it was asked for and could not show, under its entries and in its tooltip, and forgets that
note as soon as the user changes the ticks by hand.

## Outlook on the web through its compose link and the clipboard

Outlook on the web has no draft format a desktop app can hand it: an `.eml` opens in whatever the
default mail app is. Its compose link fills in the recipients, subject and a plain text body, but
cannot take HTML or an attachment. Signing in to Microsoft Graph could create a real draft with
both, but it needs an app registration, tenant consent and a token cache, which is far past what
one executable with no configuration should ask of a station. The compose link needs nothing.

So the link carries what it can, and the rest goes where the user can pick it up in one step each:
the formatted table on the clipboard as HTML (with the plain text beside it), pasted with Ctrl+V,
and the workbook saved and selected in Explorer, dragged into the message. The body is left empty
rather than filled with the plain text, which would only be pasted over. Nothing is sent from the
app, the same as with the `.eml` draft.

## Reset goes through the Recycle Bin

Reset removes the mapping CSVs along with the settings, and those are the one thing a user may have
spent real time on. Deleting them outright on one click, even a confirmed one, would make a mistake
unrecoverable, so the folder goes to the Recycle Bin through `Microsoft.VisualBasic.FileIO`, which
is part of the .NET Framework on every Windows machine and so adds no companion DLL. The window
does not save its settings while closing after a reset, which would otherwise write `settings.txt`
straight back, and the restarted process seeds everything afresh.

## Continuous integration after all

The earlier objection was that CI could neither build a WPF application nor look at the workbook.
The first half does not hold: a Windows runner builds the `net48` executable exactly as a
contributor's machine does, and runs `tools/test-core.ps1` and `tools/check-repo.ps1` against it.
The second half still does, so CI is a floor, not a replacement for looking at the report.

Releasing is where it earns its keep. The release job publishes the very executable the tests
