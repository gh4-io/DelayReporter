...
| `Core/Email` | `DelayEmail` (the compact email, from the same `ReportModel`); `EmailDraft`; `EmlDraftWriter` (MIME draft marked unsent, ported from OFT Scrubber); `OutlookWeb` (the compose link for Outlook in the browser) and `ClipboardHtml` (the table as the clipboard's HTML Format, to paste there). |
