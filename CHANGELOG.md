...
- **Send with** under Settings > Email: the default mail app as before, **Outlook on the web** for a
  work or school account, or **Outlook.com** for a personal one. The browser options open a new
  message with the recipients and subject filled in, put the formatted table on the clipboard to
  paste with Ctrl+V, and save the workbook and show it in its folder to drag into the message,
  since Outlook's compose link can carry neither.
- Help > **Reset** moves the whole `%APPDATA%\Delay Reporter` folder (settings, saved presets and the
  mapping CSVs, edits included) to the Recycle Bin after asking, then restarts Delay Reporter so it
  starts as first installed.
- Two built-in presets beside **Standard**: **All** (15 minutes of included codes, every operator)
  and **MX only** (Standard, narrowed to MX delays).
- GitHub Actions: **CI** builds and tests every branch and pull request on Windows and keeps the
  tested executable; **Release** does the same for `main` and publishes a release, with a SHA-256
  hash, whenever the project's version has not been released yet.

### Changed

- **Standard** is now exactly how a file opens, the station's own carriers ticked under Operators
  and 15 minutes of included codes. What it used to be, every operator, is the new **All** preset.
- When a preset or the default carriers ask for values the open file does not hold, the list
  itself now names them (*Also wanted, but not in this file: SIA*), in its dropdown and its
  tooltip, so "5 of 11 selected" no longer leaves two of the seven carriers unexplained.

### Fixed

- The station's default carriers ticked `S3` instead of `3S`, so AeroLogic was never ticked when a
  file opened. The seven are `3S`, `CJT`, `CKS`, `CSB`, `DHK`, `KII` and `SIA`.
